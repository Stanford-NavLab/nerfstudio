...
from regional_nerfacto.utils.geodetic_utils import geodetic_to_enu
...
class RNerfModel(NerfactoModel):
    ...
    def populate_modules(self):
        ...
        # self.query_image_path_reader = ViewerText("Query Image Path", "", cb_hook=self.path_cb)
        # self.localize_button = ViewerButton(name="Localize Query Image", cb_hook=self.localize_cb)
    ...
    def path_cb(self, element):
        self.query_image_path = element.value

    def latlon_set(self, element):
        e_proc = self.latlon_str.split(",")
        if len(e_proc) >= 2:
            # dino_lat = 37.33377
            # dino_lon = -121.88894
            dino_lat = float(e_proc[0])
            dino_lon = float(e_proc[1])
            rclat = self.field.center_latlon[0]
            rclng = self.field.center_latlon[1]
            # Convert dino latlon into ENU
            dino_east, dino_north, dino_up = geodetic_to_enu(dino_lat, dino_lon, 6378137.0, rclat, rclng, 6378137.0)
            # Convert to (1X3) tensor
            dino_enu = torch.tensor([dino_east, dino_north, dino_up])[None, :].to(self.device)
            # Convert ENU into NeRF coordinates
            self.dino_coords = self.field.enu2nerf_points(dino_enu).reshape(-1)
            self.dino_mean = None

            # Print everything
            print("Dino latlon: ", dino_lat, dino_lon)
            print("Dino ENU: ", dino_east, dino_north, dino_up)
            print("Dino NeRF: ", self.dino_coords)
            print("rc latlon: ", rclat, rclng)
    
    def latlon_cb(self, element):
        self.latlon_str = element.value

    def localize_cb(self):
        pass
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        # If dino_coords is not None, visualize the dino_coords
        if self.dino_coords is not None:
            xy = lerf_samples.frustums.get_positions()[..., :2].detach()
            
            # Convert dino_coords to same shape as xy
            dino_coords = self.dino_coords[:2].unsqueeze(0).expand_as(xy)
            
            # If xy is within epsilon of dino_coords, set mask to 1 else 0
            mask = (torch.norm(xy - dino_coords, dim=-1) < 0.05).unsqueeze(-1)
            dino_masked = torch.sum(lerf_weights * mask * lerf_field_outputs["dino"].detach(), dim=-2)
            
            # Compute outlier-robust mean of dino_masked over all except last dimension
            dino_mean = torch.mean(dino_masked.reshape(-1, dino_masked.shape[-1]), dim=0).reshape(-1, 1)

            if self.dino_mean is None:
                self.dino_mean = dino_mean
            else:
                self.dino_mean = 0.9 * self.dino_mean + 0.1 * dino_mean
            
            dino_masked = lerf_field_outputs["dino"].detach() @ self.dino_mean # rays x 1
            dino_masked /= dino_masked.norm()
            
            outputs["dino_mask"] = 0.5*torch.sum(lerf_weights * dino_masked, dim=-2) + 0.5*rgb
        ...
        delta = 1e-4
        height_vals_cur = heightcap_field_outputs["heightcap"]
        for i in range(2):
            ...
            h_xy[..., i] = (height_vals_pos.detach() - height_vals_cur).squeeze(-1) / (delta)
